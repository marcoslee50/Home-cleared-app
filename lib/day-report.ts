...
export async function generateDayReport(params: {
  ...
  let onTimeJobs = 0, earlyJobs = 0, lateJobs = 0
  ...
  const totalPhotosTaken = completedJobs.reduce(
    (s, j) => s + j.photosBefore + j.photosAfter, 0
  )
...
Return JSON only:
{
  ...
  "highlights": ["3 short bullet points - what went well or stood out"],
  "tomorrowFlags": ["any prep needed for tomorrow based on today's performance - max 3"]
}`,
      }],
    })
