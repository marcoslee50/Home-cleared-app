...
// Uses Vercel KV for serverless storage. Falls back to no-op if KV not configured.
...
export async function logDuration(clientName: string, entry: DurationEntry): Promise<void> {
  ...
  data.averageMins = Math.round(recent.reduce((a, b) => a + b.actualMins, 0) / recent.length)
