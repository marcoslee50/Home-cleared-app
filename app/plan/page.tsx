  ...
  return (
    <div className="app-shell mesh-bg">
      <header className="px-5 pt-12 pb-4">
        <Link
          href="/dashboard"
          className="text-text-muted text-sm mb-4 flex items-center gap-1 hover:text-text-secondary transition-colors"
        >
          ← Back
        </Link>
                ...
                {[true, false].map(v => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-lg font-semibold text-sm transition-all ${
                      form.barryWorking === v ? 'text-white' : 'bg-surface-muted text-text-secondary'
                    }`}
                    style={
                      form.barryWorking === v
                        ? {
                            background: v ? 'var(--brand-teal)' : 'var(--surface-muted)',
                      ...
                      {['14:00', '14:30', '15:00', '15:30', '16:00'].map(t => (
                        <button
                          ...
                          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${
                            form.barryDropoff === t ? 'text-white' : 'bg-surface-muted text-text-secondary'
                          }`}
              ...
              <div className="flex gap-2">
                {['New Brighton', 'Port Sunlight', 'Other'].map(loc => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${
                      form.startLocation === loc ? 'text-white' : 'bg-surface-muted text-text-secondary'
                    }`}
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-6">
            ...
            <div className="text-center">
              ...
              <p className="text-text-muted text-sm mt-1">
                Reading calendar · Optimising sequence · Calculating times
              </p>
            </div>
          </div>
        )}

        {state === 'ready' && plan && <PlanDisplay plan={plan} onReset={() => setState('setup')} />}
      </div>
...
function JobRow({ job, isLast }: { job: ScheduledJob; index: number; isLast: boolean }) {
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <p className="font-mono text-xs text-text-secondary flex-shrink-0">
              {job.scheduledArrival}
            </p>
          </div>
          <p className="text-text-muted text-xs mt-0.5">
            {job.jobType} · {job.estimatedDuration} min
          </p>
          {job.flags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {job.flags.map((f, i) => (
                <span key={i} className="badge badge-warn text-xs">
                  {f}
                </span>
              ))}
            </div>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-surface-border space-y-1.5">
          {job.address && <p className="text-text-secondary text-xs">📍 {job.address}</p>}
          {job.operative && (
          ...
          {job.tools.length > 0 && (
            <p className="text-text-muted text-xs">🔧 {job.tools.join(', ')}</p>
          )}
          {job.notes && <p className="text-text-muted text-xs">📝 {job.notes}</p>}
          <p className="text-text-muted text-xs">📸 Take before &amp; after photos</p>
        </div>
      )}
    </div>
  )
