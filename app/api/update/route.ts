...
export async function POST(req: NextRequest) {
  try {
    ...
    if (job && update.finishedAt) {
      ...
      if (update.notes) {
        descriptionLines.push(`📝 ${update.notes}`)
      }
      ...
      // Bug 6: job.id is the event ID, not calendar ID. Will be fixed by plumbing
      // calendarId through ScheduledJob in the bug-6 commit.
