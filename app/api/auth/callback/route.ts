...
// This route handles the Google OAuth callback during initial setup.
// After first auth, the refresh token is stored as a Vercel env var.
...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    return new NextResponse(
      `<html><body style="font-family:monospace;padding:2rem;background:#0A0F0D;color:#E8F0EB;"><h2>No code received</h2><p>OAuth flow incomplete.</p></body></html>`,
      { headers: { 'Content-Type': 'text/html' } }
    )
  }
  ...
  try {
    ...
    return new NextResponse(`
      <html><body style="font-family:monospace;padding:2rem;background:#0A0F0D;color:#E8F0EB;max-width:600px;">
        ...
        <p style="color:#D4A017;">Do NOT share this token. Add it to Vercel Environment Variables immediately.</p>
