...
function NewInvoiceForm() {
  ...
  const generatePDF = async (invoice: Record<string, unknown>): Promise<string | null> => {
    try {
      ...
      // Bug 3: this import style triggers TS errors on some module-resolution settings.
      // Will be replaced with `import autoTable from 'jspdf-autotable'` in the bug-3 commit.
      ...
      doc.setFontSize(9)
      doc.text(
        ['Port Sunlight, Wirral, CH62', '07542 888 772', 'marcos@gmx.co.uk', 'wirralgardenandproperty.com'],
        15,
        62
      )
          ...
          ...(Number(invoice.vatAmount) > 0
            ? [
                [
                  '',
                  '',
                  `VAT (${Math.round(Number(invoice.vatRate) * 100)}%)`,
      ...
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(9)
      // Bug 2: bank details read from NEXT_PUBLIC_* on the client. Will be replaced
      // with a fetch to /api/invoice-config in the bug-2 commit so the account number
      ...
      doc.text('Thank you for your business — Wirral Garden & Property', pageW / 2, 285, {
        align: 'center',
      })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
            ...
            <div key={i} className="space-y-2 pb-3 border-b border-surface-border last:border-0">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="text-text-muted hover:text-status-alert text-lg px-1"
                >
                  ✕
                </button>
              </div>
              <div className="flex gap-2">
                ...
                <input
                  ...
                  className="input-field flex-1"
                />
                <span className="text-text-secondary self-center font-semibold text-sm w-16 text-right">
        ...
        <button onClick={addItem} className="btn-secondary w-full py-2 text-sm mt-3">
          + Add line item
        </button>
        ...
        <div className="flex justify-between items-center mt-4 pt-3 border-t border-surface-border">
          ...
          <span className="font-display text-2xl font-bold" style={{ color: 'var(--brand-green)' }}>
            £{subtotal.toFixed(2)}
          </span>
        </div>
      </div>
      ...
      <button
        ...
        disabled={creating || !form.clientName}
  ...
  return (
    <div className="app-shell mesh-bg">
      <header className="px-5 pt-12 pb-4">
        <Link href="/invoices" className="text-text-muted text-sm mb-3 flex items-center gap-1">
          ← Invoices
        </Link>
